...
describe('Treasury', function () {
  ...
  beforeEach(async function () {
    ...
    ;[deployer, user, vSynthMock] = await ethers.getSigners()
    ...
    await treasury.initialize(vSynthMock.address)
  ...
  describe('pull', function () {
    ...
    it('should revert if amount == 0', async function () {
      const tx = treasury.connect(vSynthMock).pull(met.address, user.address, 0)
    ...
    it('should pull MET tokens ', async function () {
      ...
      const tx = () => treasury.connect(vSynthMock).pull(met.address, user.address, amount)
