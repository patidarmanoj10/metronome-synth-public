...
contract PoolMock is IPool, Governable, Pauseable {
    ...
    function doesSyntheticTokenExists(ISyntheticToken _syntheticToken) external view override returns (bool) {
        return address(_syntheticToken) == address(syntheticToken);
    }

    function doesDebtTokenExists(IDebtToken) external pure returns (bool) {
        return true;
    }

    function doesDepositTokenExists(IDepositToken) external pure override returns (bool) {
        return true;
    }
