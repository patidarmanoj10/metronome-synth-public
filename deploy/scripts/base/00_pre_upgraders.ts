...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const upgraders: {currentUpgrader: string; newUpgrader: string; targetProxies: string[]}[] = [
    ...
    },
    {
      ...
      targetProxies: ['SmartFarmingManager_Pool1'],
    },
    {
      currentUpgrader: 'FeeProviderUpgrader',
      newUpgrader: 'FeeProviderUpgraderV2',
      targetProxies: ['FeeProvider_Pool1'],
    },
  ]
  ...
  for (const {currentUpgrader, newUpgrader, targetProxies: proxies} of upgraders) {
    ...
    const {address: newAdmin, transactionHash} = await deploy(newUpgrader, {
      from: deployer,
      log: true,
      args: [deployer],
    })

    // Note: both base providers are failing due to same nonce usage
    // waiting for the first tx completation before broadcast a new one
    await hre.ethers.provider.waitForTransaction(transactionHash!)
